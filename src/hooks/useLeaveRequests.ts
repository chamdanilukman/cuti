...
export const transformDBToFrontend = (dbRecord: LeaveRequestDB): LeaveRequest => ({
  id: dbRecord.id,
  nama: dbRecord.nama,
  nip: dbRecord.nip,
...
const transformFrontendToDB = (frontendRecord: Omit<LeaveRequest, 'id' | 'submissionDate'>): Omit<LeaveRequestDB, 'id' | 'created_at' | 'updated_at'> => ({
  ...
  coordinator_approval_date: frontendRecord.coordinatorApprovalDate || undefined,
  admin_approval_date: frontendRecord.adminApprovalDate || undefined,
  is_revised: frontendRecord.isRevised || false,
  original_rejection_reason: frontendRecord.originalRejectionReason || undefined,
});

export const useLeaveRequests = ({ autoLoad = false }: UseLeaveRequestsOptions = {}) => {
  ...
  const updateLeaveRequest = async (id: string, updates: Partial<LeaveRequest>) => {
    try {
      ...
      if (updates.finalLetterUrl !== undefined) dbUpdates.final_letter_url = updates.finalLetterUrl;
      if (updates.coordinatorApprovalDate !== undefined) dbUpdates.coordinator_approval_date = updates.coordinatorApprovalDate;
      if (updates.adminApprovalDate !== undefined) dbUpdates.admin_approval_date = updates.adminApprovalDate;
